...
public class BlockBarDisplay extends Block {
	
	public static final PropertyDirection FACING = BlockHorizontal.FACING;
	public static final PropertyInteger DISPLAY = PropertyInteger.create("display", 0, 4);
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		...
		int display;
		if (power >= 13) display = 4;
		else if (power >= 9) display = 3;
		else if (power >= 5) display = 2;
		else if (power >= 1) display = 1;
		else display = 0;
		return state.withProperty(DISPLAY, display);
	}
